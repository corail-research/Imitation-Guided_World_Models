...
class DreamerModel(nn.Module):
    """
    Dreamer model that combines observation encoding, RSSM representation,
    and reward/pcont prediction.
    """
    def __init__(self, config):
        ...
        self.observation_encoder = Encoder(in_dim=config.IN_DIM,
                                           hidden=config.HIDDEN,
                                            embed=config.EMBED)
        self.observation_decoder = Decoder(embed=config.FEAT,
                                           hidden=config.HIDDEN,
                                           out_dim=config.IN_DIM)

        
        self.action_encoder = Encoder(in_dim=config.ACTION_SIZE,
                                      hidden=config.HIDDEN,
                                      embed=config.EMBED)
        self.final_encoder = Encoder(in_dim=2*config.EMBED,
                                     hidden=config.HIDDEN,
                                     embed=config.EMBED)
        ...
        if config.ENV_TYPE == Env.STARCRAFT:
            self.av_action = DenseBinaryModel(config.FEAT, 
                                              config.ACTION_SIZE, 
                                              config.PCONT_LAYERS, 
                                              config.PCONT_HIDDEN)
        else:
    ...
    def forward(self, observations, prev_actions=None, prev_states=None, mask=None):
        ...
        :param mask: size(batch, n_agents)
        :return: RSSMState
        """
