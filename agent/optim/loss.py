...
from agent.optim.utils import action_information_loss


def model_loss(config, model, obs, action, av_action, reward, done, fake, last, expert_action, sequential_imitation_learning_activated, hybrid_imitation_learning_activated):
    time_steps = obs.shape[0]
    batch_size = obs.shape[1]
    n_agents = obs.shape[2]

    embed_obs = model.observation_encoder(obs.reshape(-1, n_agents, obs.shape[-1]))
    embed_obs = embed_obs.reshape(time_steps, batch_size, n_agents, -1)

    if sequential_imitation_learning_activated or hybrid_imitation_learning_activated:
        embed_expert_action = model.action_encoder(expert_action.reshape(-1, n_agents, expert_action.shape[-1]))
        embed_expert_action = embed_expert_action.reshape(time_steps, batch_size, n_agents, -1)

        embed = torch.cat([embed_obs, embed_expert_action], -1)
        embed = model.final_encoder(embed.reshape(-1, n_agents, embed.shape[-1]))
...
def supervised_actor_rollout(obs, action, last, model, expert_action):
    n_agents = obs.shape[2]
    ...
    states = post.get_features()
    expert_action = expert_action[:-1]
    expert_action = expert_action.reshape(-1, action.shape[2], action.shape[-1])
    return states, expert_action
...
def hybrid_actor_rollout(obs, action, last, model, actor, critic, config):
    n_agents = obs.shape[2]
    with FreezeParameters([model]):
        embed = model.observation_encoder(obs.reshape(-1, n_agents, obs.shape[-1]))
        embed = embed.reshape(obs.shape[0], obs.shape[1], n_agents, -1)
    ...
    output = [items["actions"][:-1].detach(),
              ...
              imag_feat[:-1].detach(), 
              returns.detach()]
    return [batch_multi_agent(v, n_agents) for v in output] + [real_feat]
...
def hybrid_actor_loss(imag_states, actions, av_actions, old_policy, advantage, actor, ent_weight, expert_actions, real_feat, current_step, max_steps):
    _, new_policy_imag = actor(imag_states)
    if av_actions is not None:
        new_policy_imag[av_actions == 0] = -1e10
    actions = actions.argmax(-1, keepdim=True)
    rho = (F.log_softmax(new_policy_imag, dim=-1).gather(2, actions) -
           F.log_softmax(old_policy, dim=-1).gather(2, actions)).exp()
    ppo_loss, ent_loss = calculate_ppo_loss(new_policy_imag, rho, advantage)
    if np.random.randint(10) == 9:
        wandb.log({'Policy/Entropy': ent_loss.mean(), 'Policy/Mean action': actions.float().mean()})
    ppo_loss = (ppo_loss + ent_loss.unsqueeze(-1) * ent_weight).mean()

    _, new_policy_real = actor(real_feat)

    ce_per_item = action_information_loss(new_policy_real, expert_actions)
    loss_ce = ce_per_item.mean()

    imit_weight = 1 - (current_step / max_steps)

    return (1 - imit_weight) * ppo_loss + imit_weight * loss_ce

def supervised_actor_loss(state, targets, actor):
    _, logits = actor(state)
    target  = targets.argmax(dim=-1)
    loss_fn = torch.nn.CrossEntropyLoss()
    loss = loss_fn(logits.view(-1, 3), target.view(-1))
    return loss.mean()
