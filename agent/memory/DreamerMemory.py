...
class DreamerMemory:
    ...
    def init_buffer(self, n_agents, env_type):
        ...
        self.expert_actions = np.empty((self.capacity, n_agents, self.action_size), dtype=np.float32)
    ...
    def append(self, obs, action, reward, done, fake, last, av_action, expert_action):
        ...
        for i in range(len(obs)):
            ...
            if expert_action is not None:
                self.expert_actions[self.next_idx] = expert_action[i]
    ...
    def get_transitions(self, idxs):
        ...
        expert_action = self.process_batch(self.expert_actions, vec_idxs, batch_size)[:-1]
        ...
        return {'observation': observation, 'reward': reward, 'action': action, 'done': done, 
                'fake': fake, 'last': last, 'av_action': av_action, 'expert_action': expert_action}
