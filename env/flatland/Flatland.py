...
class FlatlandWrapper():
    ...
    def deadlocked(self):
        return self.env.deadlocked()    
    
    def arriving_not_possible(self):
        return self.env.arriving_not_possible()
    
    def arriving_before_tw_end_not_possible(self):
        return self.env.arriving_before_tw_end_not_possible()

    def not_its_target(self):
        return self.env.not_its_target()
    
    def voluntary_stops(self):
        return self.env.voluntary_stops()
...
class Flatland():
    ...
    def step(self, action_dict):
        # print("step flag 1 Flatland action_dict", action_dict)
    ...
    def reset(self):
        ...
        for agent in self.env.agents:
            self.targets.append(agent.target)
        return obs, info
    ...
    def arriving_not_possible(self):
        intdicators = []
        pcc_path = get_shortest_paths(self.env.distance_map, None)
        if self.env._max_episode_steps is not None:
            for agent in self.env.agents:
                if pcc_path[agent.handle] is None or agent.status==RailAgentStatus.DONE_REMOVED:
                    intdicators.append(0)
                else:
                    length_path_agent = len(pcc_path[agent.handle])
                    if length_path_agent + self.env._elapsed_steps > self.env._max_episode_steps:
                        intdicators.append(1)
                    else:
                        intdicators.append(0)
                
            return torch.tensor(intdicators).unsqueeze(0).unsqueeze(-1)
        return [0]*self.env.get_num_agents()
    
    def arriving_before_tw_end_not_possible(self):
        worst_malfunction_stop = 50 #self.env.stochastic_data.max_duration
        intdicators = []
        pcc_path = get_shortest_paths(self.env.distance_map, None)
        if self.env._max_episode_steps is not None:
            for agent in self.env.agents:
                if pcc_path[agent.handle] is None and agent.status==RailAgentStatus.DONE_REMOVED:
                    intdicators.append(0)
                else:
                    length_path_agent = len(pcc_path[agent.handle])
                    time_windows = worst_malfunction_stop + 3*length_path_agent
                    # print("time_windows", time_windows, "max steps", self.env._max_episode_steps)
                    if self.env._elapsed_steps > min(self.env._max_episode_steps, time_windows):
                        intdicators.append(1)
                    else:
                        intdicators.append(0)
    ...
    #     intdicators = []
    #     for agent in self.env.agents:
    #         if self.env._is_agent_ok(agent):
    #             intdicators.append(0)
    #         else:
    #             intdicators.append(1)
    #     return torch.tensor(intdicators).unsqueeze(0).unsqueeze(-1)
    def deadlocked(self):
        ...
        for i, agent in enumerate(self.env.agents):
            if agent.position in self.env.motionCheck.svDeadlocked:
                indicators.append(1)
            else:
                indicators.append(0)
        return torch.tensor(indicators).unsqueeze(0).unsqueeze(-1)
    
    def not_its_target(self):
        indicators = []
        for i, agent in enumerate(self.env.agents):
            if (agent.position in self.targets) and agent.position != agent.target:
                indicators.append(1)
            else:
                indicators.append(0)
        return torch.tensor(indicators).unsqueeze(0).unsqueeze(-1)
    ...
    def voluntary_stops(self):
        ...
        for i, agent in enumerate(self.env.agents):
            # print(agent)
            if agent.position ==  agent.old_position and agent.malfunction_data["malfunction"] == 0:
                indicators.append(1)
            else:
                indicators.append(0)
        return torch.tensor(indicators).unsqueeze(0).unsqueeze(-1)
